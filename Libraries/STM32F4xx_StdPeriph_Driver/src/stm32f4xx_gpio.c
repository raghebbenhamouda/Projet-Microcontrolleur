...
void GPIO_Init_LED(Led_Type led){
	switch(led){
	case(EVAL_ALL_LEDs) :
			RCC->AHB1ENR |= 0x00000040;//enable le port G
			GPIOG->MODER |= 0x14000000;//select mode output pour pin 13et pin 14
			GPIOG->OTYPER &=0x00009FFF ;//select pull-push type
			GPIOG->OSPEEDR|=0x3C000000;//select speed very high
			RCC->APB2ENR  |=0x00004000;//enable syscfG register
			SYSCFG->CMPCR |= 0x00000001;//enable compensation cell
			while(((SYSCFG->CMPCR) & (0x00000100)) != (0x00000100));//wait jusqu'a comp cell ready
			GPIOG->PUPDR &= 0xC3FFFFFF;//select no pull
			break;
	default:
			break;
	}
